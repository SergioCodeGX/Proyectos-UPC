...
int main() {
	...
	cout << "¿Cuantos estudiantes son adultos mayores (adulto mayor es quien tiene 60 o mas años)?: " << cont_adultos << endl;
	...
	if (cont_nivel_sat_bueno <= cont_nivel_sat_regular && cont_nivel_sat_bueno <= cont_nivel_sat_malo) {
    	cout << "Bueno" << endl;
	}

	if (cont_nivel_sat_regular <= cont_nivel_sat_bueno && cont_nivel_sat_regular <= cont_nivel_sat_malo) {
    	cout << "Regular" << endl;
	}

	if (cont_nivel_sat_malo <= cont_nivel_sat_bueno && cont_nivel_sat_malo <= cont_nivel_sat_regular) {
    	cout << "Malo" << endl;
	}
...
char leer_nivel_satisfaccion() {
	...
	} while (nivel_satisfaccion != 'B' && nivel_satisfaccion != 'R' && nivel_satisfaccion != 'M');
